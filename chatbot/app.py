...
def chat():
    ...
    try:
        ...
        # Attach tools only if the user enabled internet
        if internet_enabled:
            payload["tools"] = TOOLS
        ...
        if message_data.get("tool_calls"):
            ...
            if function_name == "search_internet":
                ...
                # Return instruction to the frontend to perform search resolution
                return jsonify({
                    "action": "search",
                    "query": search_query,
        ...
        # If no tool was called, it's a standard reply
...
def resolve_search():
    ...
    try:
        # 1. Perform actual DuckDuckGo Search
        ddgs = DDGS()
        ...
        # 2. Re-query Mistral with the new context
        messages_history = database.get_all_messages()
        api_messages = [{"role": "system", "content": SYSTEM_PROMPT}]
        api_messages.extend(messages_history)

        # Add the assistant's tool call intent
        api_messages.append({
            "role": "assistant",
            "content": "",
            "tool_calls": [{
